...
const int   mVperAmp = 185;  //mVolt->Amp scale factor
...
void setup() {
  ...
  Serial.begin(115200);
  current_char = (char*)malloc(5*sizeof(char));
  startESP();
  ...
  timer_event = timer.every(1000,pub_current, (void*)1);
}
...
void pub_current(void* context){
  ...
  hall_current = read_ACcurrent();
  //hall_current = read_DCcurrent();
  //to_Char(hall_current);
  //mqttClient.publish(MQTT_TOPIC_PUB, current_char, true);

  float floatVal= hall_current;    
  String stringVal = "";     
  
  stringVal+=String(int(floatVal))+ "."+String(getDecimal(floatVal)); 
  ...
  for(uint8_t i=0; i<sizeof(charVal);i++) Serial.print(charVal[i]);

  mqttClient.publish(MQTT_TOPIC_PUB, charVal, true);
  Serial.println("Message Published as:");
...
//to sample DC and return current 
float read_DCcurrent(){
  float result;
  int maxVal = 0;
  int minVal = 1024;
  int hall_value;
  ...
  result=((maxVal+minVal+hall_value)/3);   //avg Vdc for the second
  result=(result/1024)*5000;
  result-=offset;
  result = result/ mVperAmp; //Current RMS value
  return result;
  }
  
//to sample AC Current and return current to publish
float read_ACcurrent() {
  //hall sensor code
  ///*
  float result;
  int maxVal = 0;
...
/*void to_Char(float val) {
  for (int i = 0; i < 5; i++) {
      if (i != 1) current_char[i] = (int(val) % 10) +48;
      if (i == 1) current_char[i] = '.';
      val *= 10;
    }
}*/
